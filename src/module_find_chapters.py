...
def main(config, db, **kwargs):
    ...
    if kwargs["debug"]:
        ...
        # _update_mangaplus_manga(config, db)
        while True:
...
def _process_into_reddit_comment(config, db, youpoll, reddit_post_title, manga):
    ...
    if poll_id is not None:
        reddit_comment_body = reddit_comment_body_format.format(
            poll_url=poll_url,
            manga_title=reddit_post_title[7:],
            reddit_search_url=reddit_search_url,
            subreddit_name=manga.subreddit,
        )
    ...
    for manga in mangas:
        ...
        if resp:
            ...
            if Manga.next_update_time != manga.next_update_time:
                ...
                db.update_manga(
                    manga_id=Manga.manga_id, next_update_time=Manga.next_update_time
                )
        time.sleep(3)
