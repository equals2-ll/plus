...
def main(config,db,*args):
    if len(args) == 2 and args[1].lower()=='dump':
        warning("Dumping will overwrite manga file with database data, mainly used when transferring machines")
    ...
    for manga_id in parsed:
        if manga_id not in manga_ids:
            ...
            if resp:
                ...
                db.add_manga(manga.manga_id,manga.manga_name,manga.subreddit,manga.next_update_time,manga.is_completed,manga.is_nsfw,commit=False)
            else:
                return False
    return True
...
def _dump_to_yaml_file_from_db(db,edit_file):
    ...
    for manga in mangas:
        data[manga.manga_id]={"manga_name":manga.manga_name,"subreddit":manga.subreddit,"is_nsfw":bool(manga.is_nsfw),"is_completed":bool(manga.is_completed)}
